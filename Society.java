...
/*			if(population.size()>maxSize)
	...
	private boolean determineAbleToBreed() {
		if(population.size()<food) return true;
		else return false;
	}
//	public int maxSize(){return maxSize;}
	...
	private void gatherResources() {
		...
		cTiles.get(0).clearBlocks();
		int numparties = population.size()/5;
		
		for(int i=0;i<numparties;i++){
			Human[] members = new Human[5];
			for(int j=0; j<5; j++){
				members[i]=population.get(population.size()%i+j);
			}
			workparty.add(new WorkParty(members,cTiles.get(0)));
